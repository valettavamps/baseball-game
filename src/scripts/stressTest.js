/**
 * Game Stress Test - Plain JavaScript Version
 * Run: node src/scripts/stressTest.js
 */

const GameSimulator = require('../engine/GameSimulator').GameSimulator;

// Generate a realistic MLB-style roster
function generateTeam(teamId, teamName, seed) {
  ...
  for (let i = 0; i < 9; i++) {
    ...
    const overall = 50 + Math.floor(Math.random() * 50);
...
function generatePitcher(id, name, velocity, control) {
  return {
    id,
    name,
    position: 'P',
...
// Run stress test
async function runStressTest(gamesToSimulate = 500) {
  ...
  for (let i = 0; i < gamesToSimulate; i++) {
    ...
    totalStats.runs += result.homeTeam.score + result.awayTeam.score;
    totalStats.hits += result.homeTeam.hits + result.awayTeam.hits;
    totalStats.errors += result.homeTeam.errors + result.awayTeam.errors;
    totalStats.games++;
    ...
    if (result.innings.length > 9) totalStats.extraInnings++;
